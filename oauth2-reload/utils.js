...
const createUser = async (data) => {
  ...
  const { name, surname, nickname, login, password } = data;
  const createUserOptions = getOptionsForGetUser(name, surname, nickname, login, password);
...
const getTokenByCode = async(code) => {
  const tokenOptions = getOptionsForGetTokenByCode(code);;
  const tokenResponseFromOauth = await fetch(tokenOptions.url, tokenOptions);
  const { status : tokenStatus, statusText : tokenStatusText  } = tokenResponseFromOauth;
  if (tokenResponseFromOauth.status !== 200) throw new Error(`Error while getting token: ${tokenStatus} ${tokenStatusText}`);
  const { access_token } = JSON.parse(await tokenResponseFromOauth.text());
...
const getOptionsForLoginPage = () => ({
  ...
  params: new URLSearchParams({
      ...
      response_type : 'code',
      response_mode: 'query',
      redirect_uri : `https://${ngrok}/userinfo`,
      audience
    })
})

const getOptionsForGetTokenByCode = (code) => ({
  method: 'POST',
...
const getOptionsForGetUser = (name, surname, nickname, login, password) => ({
    email: login,
    user_metadata: {},
    blocked: false,
...
module.exports = {
  ...
  getUserById,
  getPayloadFromToken,
  getOptionsForLoginPage,
  getOptionsForGetTokenByCode,
